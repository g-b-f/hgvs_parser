...
def test_genomic_to_cdna_coord():
    ...
    for transcript_name, genomic_coord, cdna_coord_expected in _convert_coords:
        ...
        assert cdna_coord == cdna_coord_expected, repr((cdna_coord, cdna_coord_expected, transcript_name, genomic_coord))
...
def test_cdna_to_genomic_coord():
    ...
    for transcript_name, genomic_coord_expected, cdna_coord in _convert_coords:
        ...
        assert genomic_coord == genomic_coord_expected[1], repr((genomic_coord, genomic_coord_expected[1], transcript_name, cdna_coord))
...
def test_parse_name():
    ...
    for name, formatable, expected in _parse_names:
        ...
        for key, value in expected.items():
            assert getattr(hgvs_parsed, key) == value, (getattr(hgvs_parsed, key), value, name, expected)
    ...
    for (expected_hgvs_name, variant,
        ...
        if name_canonical:
            ...
            hgvs_name = format_hgvs_name(
                ...
                use_gene=False)
            assert hgvs_name == expected_hgvs_name, repr([hgvs_name, expected_hgvs_name, variant])
